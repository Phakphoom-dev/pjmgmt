      ...
      <v-col>
        ...
        >
          ...
          <template v-slot:item.actions="{ item }">
            <v-icon class="mr-2" @click="manageLesson(item)" color="info">
              mdi-pencil
            </v-icon>
          </template>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "จัดการบทเรียน", value: "actions", sortable: false },
    ],
    subjects: [],
    teachers: [],
  }),
  ...
  methods: {
    ...
    async initialize() {
      ...
      if (userData.role === "teacher") {
        this.subjects = await this.get("/subject/getTeacherSubjects.php");
        this.subjects = this.subjects.filter(
          (subject) => subject.teacherId == userData.adminId
        );
        ...
        localStorage.setItem("subjectIds", subjectIds);
      }
    ...
    manageLesson(item) {
      this.$router.push({
        name: "ShowLessons",
        query: { subjectId: item.subjectId, subjectName: item.subjectName },
      });
    },
