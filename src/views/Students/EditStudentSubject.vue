<template>
  <v-container fluid class="mt-3">
    ...
    >
    <v-card class="mt-3">
      <v-card-title>แก้ไขหลักสูตรผู้เรียน</v-card-title>
      <v-card-text>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$refs.observer.validate().then((result) => {
        if (result) {
          ...
          this.$http
            .post(
              `${process.env.VUE_APP_API_PATH}/subject/editStudentSubject.php`,
              formData
            )
            .then((res) => {
    ...
    chooseStd() {
      ...
      this.subjectForm.subjectId = parseInt(this.$route.query.subjectId);
      this.subjectForm.objective = this.$route.query.objective;
      this.subjectForm.dates[0] = this.$route.query.startDate;
      this.subjectForm.dates[1] = this.$route.query.endDate;
      this.dateStartMenu = true;
      this.dateStartMenu = false;
      this.dateEndMenu = true;
