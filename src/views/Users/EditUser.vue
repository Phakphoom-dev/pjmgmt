<template>
  <v-container fluid class="mt-3">
    ...
    >
    <v-card class="mt-3">
      <v-card-title> แก้ไขผู้ใช้งาน</v-card-title>
      <v-card-text>
...
export default {
  ...
  methods: {
    getAdmin() {
      const jsonData = JSON.stringify({ adminId: this.$route.query.adminId });

      this.$http
        .post(`${process.env.VUE_APP_API_PATH}/user/getAdmin.php`, jsonData)
        .then((res) => {
    ...
    async submit() {
      ...
      if (isValid) {
        ...
        this.$http
          .post(
            `${process.env.VUE_APP_API_PATH}/user/updateAdmin.php`,
            formData
          )
          .then((res) => {
            if (res.status === 200) {
